...
async function main() {
  ...
  for (const line of text.split("\n")) {
    if (line.startsWith("data: ")) {
      jwt = JSON.parse(line.slice(6)).result.structuredContent.session_jwt; break;
    }
  }
  ...
  await conn.run("USE remote;");

  const tests: [string, string][] = [
    ["FROM sales.orders", "FROM sales.orders LIMIT 2"],
    ["FROM sales.customers", "FROM sales.customers LIMIT 1"],
    ["SELECT * FROM sales.orders", "SELECT * FROM sales.orders LIMIT 2"],
    ["SELECT ssn FROM sales.customers", "SELECT ssn FROM sales.customers LIMIT 1"],
    ["SELECT name, email FROM sales.customers", "SELECT name, email FROM sales.customers LIMIT 1"],
  ];
