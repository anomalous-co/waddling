...
export async function bootDuckRuntime(config) {
    ...
    const opts = [`DATA_PATH ${q(config.ducklakeDataPath)}`];
    if (config.encrypted)
        opts.push("ENCRYPTED");
    const attachSql = `ATTACH '${catalogTarget}' AS ${config.lakeAlias} (${opts.join(", ")})`;
    {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                lastErr = err;
                console.log(`[gateway] ATTACH ducklake attempt ${attempt}/${maxAttempts} failed: ${err instanceof Error ? err.message : String(err)}`);
    ...
    await connection.run(`CALL quack_serve('quack:localhost:${config.quackPort}', token := ${q(config.serverToken)})`);
    const runtime = {
        connection,
        config,
        async run(sql) {
 ...
 */
export async function describeTables(rt, only) {
    const reader = await rt.connection.runAndReadAll(`SELECT schema_name, table_name, column_name, data_type, is_nullable
       FROM duckdb_columns()
    ...
    for (const r of rows) {
        ...
        if (wanted && !wanted.has(key.toLowerCase()))
            continue;
 ...
 */
export async function applySnapshot(rt, snapshot, auth) {
    ...
    for (const rc of snapshot.roleConstraints ?? []) {
        ...
        await c.run(`SELECT birdshot_add_grant_constraint(${q(rc.role)}, ${q(rc.tableRef)}, ${q(cols)}, ${q(ws)}, ${q(we)})`);
    }
    await c.run("SELECT birdshot_commit_config()");
}
/** Instant revocation → in-memory denylist; next query for the subject denied. */
export async function birdshotRevoke(rt, kind, id, reason, expiresUs) {
...
export async function ducklakeSnapshot(rt) {
    try {
        const reader = await rt.connection.runAndReadAll(`SELECT * FROM ${rt.config.lakeAlias}.current_snapshot()`);
