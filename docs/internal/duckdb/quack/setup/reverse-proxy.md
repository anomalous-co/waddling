...
The Quack server speaks plain HTTP only and binds to localhost by default. See [Security](../security.md) for the rationale behind these configurations. For any deployment beyond local-only, the recommended pattern is the same as for any other HTTP-based database / application server: put a proven HTTP reverse proxy in front of it, and let the proxy terminate TLS.
...
```bash
certbot --nginx -d quack.example.com
```
