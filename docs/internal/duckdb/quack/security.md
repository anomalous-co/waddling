...
- The server does not use TLS itself. Involving TLS for localhost communication only adds dependencies for no real benefit.

### Best Practice

For any deployment beyond local-only, do **not** expose Quack directly to the internet. We recommend putting a proven HTTP reverse proxy in front of it and letting the proxy terminate TLS.

The Quack client is shipped with these assumptions in mind: for non-local URIs it assumes HTTPS by default, so a properly fronted server "just works" from the client side too. See *Securing Quack with a Reverse Proxy* for nginx and Caddy recipes (production and local-test).
...
Quack runs these as two separate hooks: authentication when a client first connects, and authorization before each query the client wants to issue.

### Default Configuration

Both hooks ship with built-in defaults suitable for local development and single-user deployments. Each is exposed as an overridable callback for deployments with stricter requirements.

Out of the box:

- **Authentication** is token-based. When you call `quack_serve`, the server generates a random token and returns it in the `auth_token` column (or you can supply one explicitly via `quack_serve(uri, token := '...')`). Clients must present this token on every connection, either through a Quack secret scoped to the server URI or via the explicit `TOKEN` option in the `ATTACH` statement / `quack_query` function call. The default authentication callback compares the client-supplied token against the server's stored token.
...
Both calls must return a `BOOLEAN`: `true` admits the request; anything else (including a query error) rejects it with *Authentication failed* / *Authorization failed*. Anything resolvable as a function with the matching arity and returning a `BOOLEAN` will work: built-in scalar functions, scalar UDFs registered by another extension, or SQL macros. Authentication takes `(VARCHAR, VARCHAR, VARCHAR)`; authorization takes `(VARCHAR, VARCHAR)`.
...
| -------------- | ------------------------------------------------------------------------------------------- |
| `session_id`   | Server-generated session id (random 32-character string). Becomes `quack_connection_id` for that client. |
...
The cleanest way to plug in custom authentication is through a `MACRO`.
...
When developing locally in a sandboxed environment, you can use "developer mode" authentication, which allows every incoming connection:
...
| Argument        | Description                                                                                     |
| --------------- | ----------------------------------------------------------------------------------------------- |
| `connection_id` | The `quack_connection_id` of the calling client (i.e., the same id the authentication hook saw as its `session_id`). |
| `query`         | The full SQL text the client wants to execute.                                                  |

#### Overriding Authorization

Authorization runs once per `PREPARE_REQUEST`, with the connection id and the full SQL text. Common patterns:
...
For more involved authorization functions, see *Beyond SQL Macros* below.
...
> **Note:** Python UDFs registered through `con.create_function` are scoped to the connection that created them. Quack invokes each callback on a fresh server-side connection, so Python UDFs are not visible at dispatch time and cannot be used as authentication or authorization callbacks. Register the function via a DuckDB extension to make it globally visible.
...
### Per-User Access Control List

To implement a per-user access control list (ACL), create a custom authentication hook that records `sid → user` pairs so authorization can look up who is asking. Because macros can't write, the recording side must be a scalar UDF defined by a custom DuckDB extension. The authorization side can be a macro:
