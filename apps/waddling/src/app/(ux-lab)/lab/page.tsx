...
function LiveSessionsCard() {
  ...
  useEffect(() => {
    let cancelled = false;
    void fetchCp<{ sessions: SessionRow[] }>('/api/cp/sessions').then((res) => {
      ...
      headerActions={
        sessions && sessions.length > 0 ? (
          <Button variant="ghost" size="sm" asChild>
            <Link href="/lab/agents">View all</Link>
          </Button>
        ) : undefined
      }
    >
...
function YourAgentsCard() {
  ...
  return (
    <SectionCard
      ...
      headerActions={
        <Button variant="ghost" size="sm" asChild>
          <Link href="/lab/agents">Manage</Link>
        </Button>
      }
    >
      {agents === null ? (
 ...
 * Above the fold: hero CTA, stat pills, live sessions table, agent roster preview.
 */
