...
export function SignUpForm() {
  ...
  // A validated same-origin return target — e.g. the MCP device-link claim (/link?code=…)
  // that funneled this user here. It survives account creation by riding through onboarding
  // (see onboardingUrl below), and is threaded onward to the sign-in link.
  ...
  // origin (api.*) instead of the app. /onboarding creates the org + starts the trial —
  // none of which can run at sign-up time, because requireEmailVerification means
  // sign-up returns no session until the email is verified. When a `next` is present
  // (MCP funnel), carry it THROUGH onboarding so the user returns to the claim once
  // their org exists — onboarding forwards to `next` on completion.
  const onboardingUrl = () => {
    const origin = typeof window !== 'undefined' ? window.location.origin : '';
    const query = safeNext ? `?next=${encodeURIComponent(safeNext)}` : '';
    return `${origin}/onboarding${query}`;
  };
  ...
  const submitAccount = async (e: FormEvent) => {
    ...
    // dispatched a verification email. Org creation + trial happen in /onboarding,
