/**
 * Gateway control-channel HTTP client (W1 → W3 contract).
 *
 * The per-org DuckDB gateway (`packages/gateway`, W3) exposes a control port
 * (ctrl 9510 in the demo, §8) that the control plane drives:
 *
 ...
 * defined locally (re-using @/lib/types where a shared shape exists) because they
 * are a control-channel wire contract, not a domain model.
 */
...
export function gatewayClientFor(endpoint: {
  ...
  const ctrlPort =
    endpoint.ctrl_port ?? (endpoint.quack_port ? endpoint.quack_port + 10 : 9510);
  return new GatewayClient({
    baseUrl: `http://${host}:${ctrlPort}`,
