/**
 * Endpoint credential store + gateway-config accessor (migration 005).
 *
...
import { sealJson, openJson, type SealedSecret } from './secret-crypto';
...
export async function getEndpointGatewayConfig(
  ...
  if (!localData && ep.storage_provider) {
    ...
    const creds =
      provider === 'config' && byKind.has('storage')
        ? openJson<StorageCreds>(toSealed(byKind.get('storage')!))
        : { keyId: '', secret: '' };
    s3 = {
      provider,
  ...
  if (ep.catalog_mode === 'byo-postgres' && byKind.has('catalog')) {
    ducklakeCatalogDsn = openJson<CatalogCreds>(toSealed(byKind.get('catalog')!)).dsn;
  }
