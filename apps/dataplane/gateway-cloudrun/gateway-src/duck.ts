...
/**
 * Append mTLS material to a libpq key=value catalog DSN for the postgres-catalog ATTACH.
 * Cloud SQL requires a client cert (TRUSTED_CLIENT_CERTIFICATE_REQUIRED) and presents a
 * per-instance internal CA that is NOT in the system trust store, so the org DSN's
 * `sslmode=verify-ca` needs an explicit sslcert/sslkey/sslrootcert. The cert is shared per
 * deployment (paths materialized by the entrypoint from secrets). Each key is appended only
 * when configured AND not already present — a no-op for local-file/demo catalogs and safe if
 * a DSN ever already carries one. Paths are deployment-controlled (no spaces), so the bare
 * key=value form needs no quoting.
 */
function withCatalogSsl(dsn: string, cfg: GatewayConfig): string {
  const parts = [dsn];
  const add = (key: string, val: string) => {
    if (val && !new RegExp(`(^|\\s)${key}=`).test(dsn)) parts.push(`${key}=${val}`);
  };
  add("sslcert", cfg.catalogSslCert);
  add("sslkey", cfg.catalogSslKey);
  add("sslrootcert", cfg.catalogSslRootCert);
  return parts.join(" ");
}
...
export interface DuckRuntime {
  connection: DuckDBConnection;
  /** The underlying instance, so callers can open additional connections (reader pool). */
 ...
 *
 * opts.serveQuack (default true) — when false, skip `quack_serve` + the quack auth/authz
 * SET GLOBALs. The Cloud Run gateway authorizes via `birdshot_authorize` directly on this
 * connection (the governed-load pattern) and never serves quack, so it boots with serveQuack
 * false: no quack listener, and the lake handle has no quack/view dependents.
 * opts.lakeViews (default true) — when false, skip the `memory.main` read-through views. Only
 * quack serving needs them; reading the lake handle directly (`lake.main.<t>`) does not, and
 * skipping them removes the cross-replica view-staleness that made post-write reads return 0.
 */
export async function bootDuckRuntime(
  config: GatewayConfig,
  opts: { serveQuack?: boolean; lakeViews?: boolean } = {},
): Promise<DuckRuntime> {
  const serveQuack = opts.serveQuack !== false;
  const lakeViews = opts.lakeViews !== false;
  ...
  // quack (HTTP catalog federation) + birdshot (ACL enforcement). quack is only needed when
  // serving the wire protocol; the Cloud Run gateway (serveQuack=false) authorizes via
  // birdshot_authorize directly, so it skips quack entirely (no custom-repo dependency).
  // birdshot does NOT depend on quack being loaded.
  if (serveQuack) await connection.run("INSTALL quack; LOAD quack");
  ...
  if (config.quackboard) {
    ...
    console.log(
      `[gateway] workspace database name: ${dbName}, lakeAlias=${config.lakeAlias || "(empty)"}`,
    );
    await connection.run(`USE ${qid(dbName)}`);
    config.lakeAlias = dbName;
    mark("workspace-open");
  ...
  // No-lake modes (quackboard, workspace) skip the S3 secret, ducklake ATTACH, and read-through
  // views. quack serves the opened database's own tables directly (agent DDL persisted there).
  // birdshot hooks + quack_serve below still run, so ACLs are enforced.
  if (!config.quackboard && !config.workspaceMode) {
  ...
  // Append the shared mTLS cert/key/rootcert to the postgres-catalog DSN (Cloud SQL). The
  // augmented DSN is reused for both the metadata-schema bootstrap ATTACH and the lake ATTACH.
  const catalogDsn = isPgCatalog ? withCatalogSsl(config.ducklakeCatalogDsn, config) : config.ducklakeCatalogDsn;
  const catalogTarget = isPgCatalog
    ? `ducklake:postgres:${catalogDsn}`
  ...
  if (useMetadataSchema) {
    await ensureCatalogSchema(connection, catalogDsn, config.metadataSchema);
  ...
  console.log(`[gateway:boot] bootDuckRuntime ready in ${Date.now() - tBoot}ms (serveQuack=${serveQuack})`);
...
export async function applySnapshot(
  ...
  // Build the ENTIRE birdshot config as one ';'-separated program and apply it in a
  // single connection.run() round-trip. Previously every jwk/role/grant/constraint/policy
  // was its own awaited c.run(), so a wildcard-expanded snapshot over a large catalog ×
  // many agents meant hundreds of serial round-trips on every re-arm — the dominant
  // apply-side cost when access/url rules change. @duckdb/node-api run(sql) without bind
  // params executes multiple ';'-separated statements in one native query (the same path
  // already used for "INSTALL quack; LOAD quack"), and birdshot stages every setter under
  // one mutex then promotes staging→live on commit, so a single program is still atomic.
  const stmts: string[] = ["SELECT birdshot_reset_config()"];
  ...
  if (rt.config.lakeAlias) {
    stmts.push(`SELECT birdshot_set_lake_catalog(${q(rt.config.lakeAlias)})`);
  }
  if (auth) {
    stmts.push(`SELECT birdshot_set_auth(${q(auth.issuer)}, ${q(auth.audience)}, 'rs256')`);
    for (const k of auth.jwks ?? []) {
      stmts.push(`SELECT birdshot_add_jwk(${q(k.kid)}, ${q(k.n)}, ${q(k.e)})`);
    }
  }
  for (const ur of snapshot.userRoles) {
    stmts.push(`SELECT birdshot_add_user_role(${q(ur.userId)}, ${q(ur.role)})`);
  }
  for (const g of snapshot.roleGrants) {
    stmts.push(`SELECT birdshot_add_role_grant(${q(g.role)}, ${q(g.tableRef)}, ${q(g.action)})`);
  }
  ...
  for (const p of snapshot.policies ?? []) {
    ...
    stmts.push(`SELECT ${fn}(${q(p.role)}, ${q(p.pattern)})`);
  }
  stmts.push("SELECT birdshot_commit_config()");

  await c.run(stmts.join(";\n"));
  ...
  // table queryable on whichever replica serves the next read. No-op for no-lake modes.
  //
  ...
  if (!rt.config.quackboard && !rt.config.workspaceMode) {
    await Promise.race([
